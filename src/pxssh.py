...
class pxssh (spawn):
    ...
    def __init__ (self, timeout=30, maxread=2000, searchwindowsize=None, logfile=None, cwd=None, env=None):
        ...
        # disabling this so that we can use RSA public keys
...
#        return False
    ...
    def synch_original_prompt(self):
        ...
        time.sleep(5)
        self.sendline()
        #return pxssh.pxssh.synch_original_prompt(self)
        max_attempts = 4
        #log.debug("Synch_original_prompt function")
        last = self.read_nonblocking(size=1000,timeout=1)
        for i in xrange(max_attempts):
            time.sleep(0.5)
            self.sendline()
            time.sleep(0.5)
            val = self.read_nonblocking(size=1000,timeout=1)
            #log.debug("Try %d: %s" % (i + 1, val))
            if self.line_match(last, val):
                return True
            last = val
        #log.warn("Didn't match prompt in %d attempts" % max_attempts)
        return False

    def line_match(self, a, b):
        ld = self.levenshtein_distance(a,b)
        len_a = len(a)
        if len_a == 0:
            return False
    ...
    def login (self,server,username,password='',terminal_type='ansi',original_prompt=r"[#$]",login_timeout=10,port=None,auto_prompt_reset=True):
        ...
        i = self.expect(["(?i)are you sure you want to continue connecting", original_prompt, "(?i)(?:password)|(?:passphrase for key)",
                         "(?i)permission denied", "(?i)terminal type", TIMEOUT, "(?i)connection closed by remote host","Host key verification failed."], timeout=login_timeout)
        ...
        if i==0:
            ...
            ######### original comments no longer apply!!! #####################
            ...
            #pass
            self.close()
            raise ExceptionPxssh ("Timeout during Login")
        elif i==6: # Connection closed by remote host
            self.close()
            raise ExceptionPxssh ('connection closed')
        elif i == 7:
            self.close()
            raise ExceptionPxssh ("Host key verification failed (stale key?)")
        else: # Unexpected
            self.close()
